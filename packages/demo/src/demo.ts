...
class Database implements KVH.DB.Database {
  ...
  async startTransaction(height: number): Promise<KVH.DB.Database.Transaction> {
    return this._engine.createTransaction(height);
  }
  ...
  hook(
    ...
    transactionCallback: (
      trs: KVH.DB.Database.Transaction<
        KVH.Base.Type.Collection.Item<KVH.Base.Key.Type, KVH.Base.Value.Type>
      >
    ) => void
  ): Promise<void> {
  ...
  ): Promise<KVH.DB.Database.Core.KVHInfo> {
    throw new Error("Method not implemented.");
  }
  subscribe(
    key: KVH.Base.Key.Type,
    candidates?: KVH.Base.Key.Candidate.Type[]
  ): KVH.DB.Database.Core.Subscriber {
    throw new Error("Method not implemented.");
  }
